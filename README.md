...
```bash
git clone https://github.com/sirraya-labs/m-nsta.git
cd m-nsta
...
```python
from mnsta import SirrayaInspector

# Initialize the M-NSTA Forensic Engine
PI_Inspector = SirrayaInspector(mode="high_certainty")

# Analyze video for Spatial and Temporal integrity
results = PI_Inspector.analyze("source_video.mp4")

if results.is_authentic:
    print(f"Verified: {results.confidence_score}% - Heartbeat: {results.bpm} BPM")
else:
    print(f"Warning: {results.flag_reason}")
...
* [x] Launch PI Manifesto and Core README.
* [ ] Release Neural Layer (Density Matrix logic).
* [ ] Release Spatial Layer (3D Manifold Mapping).
* [ ] Integrate rPPG Temporal Layer.
* [ ] Full UDNA Cryptographic Signing Integration.

---

## 📜 Citation

If you use M-NSTA in your AI Safety or Forensics research, please cite the Principal Investigator:

```bibtex
@manual{MNSTA_Sirraya_2026,
  title  = {M-NSTA: Multi-Modal Neural-Spatial-Temporal Analysis Framework},
  author = {Amir Hameed Mir, Principal Investigator, Sirraya Labs},
  year   = {2026},
  url    = {https://github.com/sirraya-labs/m-nsta}
}
